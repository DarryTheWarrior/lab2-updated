...
public class token_list {
     ...
     */
    public static double expr(token_list token0) {
     ...
     */
    public static double plusminus(token_list token0) {
        double value = multdiv(token0);
        while (true) {
            token token1 = token0.next();
            switch (token1.type) {
                case OP_PLUS:
     ...
     * @return calculation of an expression between adjacent numbers according to the token between them
     */
    public static double multdiv(token_list token0) {
        double value = factor(token0);
        while (true) {
            token token1 = token0.next();
            switch (token1.type) {
                case OP_MUL:
     ...
     * @return calculation of an expression between adjacent numbers according to the token between them
     */
    public static double factor(token_list token0) {
        token token1 = token0.next();
        switch (token1.type) {
            case NAME:
                token0.back();
            ...
            case OP_MINUS:
                ...
                return -value;
            case NUMBER:
              return Double.parseDouble(token1.value);
            case LEFT_BRACKET:
     ...
     */
    public static double func(token_list token_list) {
